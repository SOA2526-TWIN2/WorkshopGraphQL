...
public class Mutations implements GraphQLRootResolver {
    ...
    public Boolean addModule(String matricule , String libelle,
                             int coef, int volumeHorraire,
                             int codeUE, Module.TypeModule type ){
        UniteEnseignement ue= ueb.getUEByCode(codeUE);
        Module m = new Module(matricule,libelle,coef,volumeHorraire,
                type,ue);
        return  mb.addModule(m);
    }

    public Module UpdateModule(String matricule , String libelle,
                               int coef, int volumeHorraire,
                               Module.TypeModule type,int codeUE){
        UniteEnseignement ue= ueb.getUEByCode(codeUE);
        Module moduleupdated= new Module(matricule,libelle,
                coef,volumeHorraire,type,ue);
        mb.updateModule(matricule,moduleupdated);
